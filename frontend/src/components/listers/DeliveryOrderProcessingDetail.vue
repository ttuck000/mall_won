<template>
    <v-card outlined>
        <v-card-title>
            DeliveryOrderProcessing # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="OrderAmout" v-model="item.orderAmout" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/deliveryOrderProcessings/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
